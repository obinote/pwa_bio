...
const { getAppEnv, getAccessKey, getEncryptKey } = require('../../../helpers/env');
...
// for start running when yarn local
const generateConfigAuto = async () => {
    try {
        const response = await graphQLClient.request(reqBody, {});
        if (response) {
            fs.writeFile(`${baseDir}config.json`, JSON.stringify(response), (err) => {
                if (err) throw err;
            });
            console.log('[success] generate config');
        }
    } catch (err) {
        console.log('[error] generate config', err);
    }
};

// for url/generate-config
const generateConfig = (req, res) => {
    if (`Bearer ${getEncryptKey()}` == req.headers.authorization) {
        graphQLClient
            .request(reqBody, {})
            .then((data) => {
...
module.exports = { generateConfig, generateConfigAuto };
