...
import {
    graphqlEndpoint, OMSGraphqlEndpoint, HOST, storeCode,
...
const uriOMS = OMSGraphqlEndpoint[appEnv] ? OMSGraphqlEndpoint[appEnv] : OMSGraphqlEndpoint.dev;
...
const logoutLink = onError((err) => {
    ...
    if (networkError && typeof window !== 'undefined' && graphQLErrors && graphQLErrors.length > 0 && graphQLErrors[0].status > 500) {
        ...
        if (!graphQLErrors[0].extensions.category === 'graphql-authorization' && !graphQLErrors[0].status === 401) {
...
const link = new RetryLink().split(
    ...
    new RetryLink().split(
        (operation) => operation.getContext().request === 'oms',
        new HttpLink({
            uri: uriOMS, // Server URL (must be absolute)
            credentials: 'same-origin', // Additional fetch() options like `credentials` or `headers`
            fetch,
