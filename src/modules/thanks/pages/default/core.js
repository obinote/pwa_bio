...
import { getOrder, getPaymentInformation } from '@core_modules/thanks/services/graphql';
...
const PageStoreCredit = (props) => {
    ...
    const { data: paymentInformation, loading: paymentLoading, error: paymentError } = getPaymentInformation(
        typeof checkoutData === 'string'
            ? {
                order_number: JSON.parse(checkoutData)?.order_number ?? null,
            }
            : {
                order_number: checkoutData?.order_number ?? null,
            },
    );
    ...
    if (loading || !data || paymentLoading || !paymentInformation) {
        return (
    ...
    const handleDetailSign = () => {
        deleteCheckoutData();
        Router.push('/digitalsign/[orderNumber]', `/digitalsign/${checkoutData.order_number}`);
    }

    if (data && data.ordersFilter && data.ordersFilter.data.length > 0 && paymentInformation) {
        ...
        return (
            <Layout t={t} {...other} pageConfig={config} storeConfig={storeConfig} showRecentlyBar={false}>
                <Content
                    ...
                    handleDetailSign={handleDetailSign}
    ...
    return <ErrorInfo variant="warning" text={t('common:error:notFound')} />;
};
