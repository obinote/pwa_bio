...
const Banner = ({
    ...
    const classCarousel = customClassCaraousel && customClassCaraousel !== '' ? customClassCaraousel : styles.caraousel;

    const customProductCaraosel = customProduct && customProduct !== '' ? customProduct : styles.customClass;
    ...
    return (
        <div className={classNames(styles.container, mode)}>
            {thumbnail && mode === 'vertical' ? (
                <div>
                    {data.map((item, id) => (
                        <div
                            className={
            ...
            {thumbnail && mode === 'horizontal' ? (
                <div className={classNames(styles.thumbnailHolder, mode)}>
                    {data.map((item, id) => (
                        <div
                            className={
                                slideIndex === id
                                    ? classNames(styles.thumbnail, styles.thumbnailActive, mode, 'hidden-mobile')
                                    : classNames(styles.thumbnail, mode, 'hidden-mobile')
                            }
                            key={id}
                            onClick={() => {
                                sliderRef.slickGoTo(id);
                        ...
                        >
                            <Thumbor
                                ...
                                height={100}
                                quality={100}
                                className={classNames(styles.thumbnailImg, mode)}
                                videoUrl={item.videoUrl}
                                widthMobile={width || defaultWidthMobile}
                                heightMobile={height || defaultHeightMobile}
                            />
