...
const OrderStatusIcon = (props) => {
    ...
    const styles = useIconStyles();
    let steps = ['pending', 'processing', 'shipping', 'delivered', 'complete'];
    if (status === 'canceled') {
        steps = ['pending', 'canceled'];
    }

    if (status === 'refunded') {
        steps = ['pending', 'processing', 'shipping', 'complete', 'refunded'];
    }

    if (status === 'partially_refunded') {
        steps = ['pending', 'processing', 'shipping', 'complete', 'partially_refunded'];
    }
    ...
    return (
        <div className={styles.container}>
            <Stepper className={styles.alternativeLabel} alternativeLabel activeStep={steps.indexOf(status)} connector={<CustomConnector />}>
                {steps.map((label) => (
                    <Step key={label}>
                        <StepLabel
