...
import { getOrderForPrint, getPaymentInformation } from '@core_modules/order/services/graphql';
...
const OrderPrint = (props) => {
    ...
    const { loading, data } = getOrderForPrint(params);
    if (!loading && data && data.customer.orders) {
        // eslint-disable-next-line prefer-destructuring
        detail = data.customer.orders.items;
    }
    const currency = detail.length > 0 ? detail[0].global_currency_code : storeConfig.base_currency_code;

    const { data: paymentInfo } = getPaymentInformation(params);
