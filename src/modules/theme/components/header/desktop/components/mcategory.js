...
const Menu = (props) => {
    const { data, storeConfig, forcedShowMenu = false } = props;
    const [showMenu, setShowMenu] = React.useState(forcedShowMenu);
    const cmsPages = storeConfig && storeConfig.cms_page ? storeConfig.cms_page.split(',') : [];
    let menu = storeConfig.pwa.ves_menu_enable
        ? data?.vesMenu?.items
        : data?.categoryList ? data?.categoryList[0]?.children : [];
    if (!menu) {
        menu = [];
    }
    const categoryMenu = menu.find((menuItem) => !menuItem.link);
    ...
    const handleClickCategory = () => {
        ...
        const getClassLink = document.getElementsByClassName('mega-menu');
        for (let i = 0; i < getClassLink.length; i += 1) {
            getClassLink[i].style.overflow = 'visible';
            getClassLink[i].style.visibility = 'visible';
        }
    };

    React.useEffect(() => {
        setShowMenu(forcedShowMenu);
    }, [forcedShowMenu]);
    ...
    return (
        <div className="menu-wrapper" role="navigation">
            {forcedShowMenu ? (
                <>
                    {categoryMenu?.children?.length > 0 && (
                        <div className="mega-menu row" aria-hidden="true" role="menu">
                            <MenuChildren data={categoryMenu.children} handleClick={handleClick} generateLink={generateLink} />
                        </div>
                    )}
                </>
            ) : (
                            ...
                            return (
                                <li key={idx} role="menuitem">
                                    {val.link ? (
                                        ...
                                        <a href="#" className={val.children.length > 0 && showMenu ? 'menu-opened category-menu' : 'category-menu'} onClick={() => handleClickCategory(val)} dangerouslySetInnerHTML={{ __html: val.name }} />
                                    ) }

                                    {val.children.length > 0 && showMenu ? (
                                        <div className="mega-menu row" aria-hidden="true" role="menu">
            ...
            <style jsx global>
                {`
                    ...
                    .category-menu.menu-opened, .category-menu:hover {
