            ...
            <div className="container">
                {data.map((val, idx) => (
                    <div className="col-lg-2">
                        <Link key={idx} href={generateLink(val)[0]} as={generateLink(val)[1]}>
                            <a onClick={() => handleClick(val)} className="category-lvl2" onMouseEnter={() => setActive(idx)}>
                                {val.name}
                            </a>
                        </Link>
                        <div className="row container-lvl3">
                            {val.children.map((lvl3) => (
                                <Link href={generateLink(lvl3)[0]} as={generateLink(lvl3)[1]}>
                                    <a onClick={() => handleClick(lvl3)} className="col category-lvl3">
                                        {lvl3.name}
                                    </a>
                                </Link>
                            ))}
