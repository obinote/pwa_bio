...
const Checkout = (props) => {
    ...
    const [isApplyEligibleCoupon, setApplyEligibleCoupon] = useState(false);

    const [setCheckoutSession] = gqlService.setCheckoutSession();
    const [checkoutTokenState, setCheckoutTokenState] = useState();
    const [setEligibleCouponToCart] = gqlService.applyEligibleCouponToCart();
    ...
    useEffect(() => {
        if (cartId) {
            ...
            setEligibleCouponToCart({
                variables: {
                    cart_id: cartId || propsCardId,
                },
            }).then(async () => {
                setApplyEligibleCoupon(true);
            })
                .catch((e) => {
                    console.log(e);
                });
    ...
    const [loadingShippingAddress, setLoadingShippingAddress] = React.useState(true);
    ...
    const CheckoutSchema = Yup.object().shape({
        ...
        // input_purchase_letter: !checkout.input_purchase_letter ? Yup.object().nullable().required(t('checkout:required')) : null,
        upload_purchase_letter: !checkout.upload_purchase_letter ? Yup.object().nullable().required(t('checkout:required')) : null,
        signature: !checkout.selected.signature && !checkout.selected.checkoutOtp ? Yup.object().nullable().required(t('checkout:required')) : null,
        checkoutOtp: !checkout.selected.signature && !checkout.selected.checkoutOtp ? Yup.number().nullable().required(t('checkout:required')) : null,
    ...
    const initData = () => {
        ...
        if (!state.data.isGuest && manageCustomer && manageCustomer.data && manageCustomer.data.customer && manageCustomer.data.customer.addresses) {
            ...
            [addressBilling] = customer ? customer.addresses.filter((item) => item.default_billing) : [null];
        }
        ...
        if (shipping && !state.selected.address) {
        ...
        const billingAddress = cart && cart.billing_address ? cart.billing_address : null;
        if (billingAddress) {
            state.selected.billing = {
                ...
                address_label: billingAddress.address_label,
            };
        }
    ...
    useEffect(() => {
        ...
        if (loadCart && cartId && isApplyEligibleCoupon) {
        ...
        if (dataCart && dataCart.cart && itemCart && itemCart.cart && cartId && isApplyEligibleCoupon) {
            initData();
        }
    }, [manageCustomer.data, dataCart, itemCart, cartId, errorCart, errorItem, isApplyEligibleCoupon]);
    ...
    React.useEffect(() => {
        ...
        if (
            ...
            [addressBilling] = customer ? customer.addresses.filter((item) => item.default_billing) : [null];
            state.data.defaultAddress = customer ? address : null;
            state.data.defaultBillingAddress = customer ? addressBilling : null;
