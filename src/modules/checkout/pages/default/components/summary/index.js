...
// import ModalBriCeria from '@core_modules/checkout/pages/default/components/ModalBriCeria/index';
...
const Summary = ({
    ...
    // const [insertSpNumber] = gqlService.insertSpNumber({ onError: () => {} });
    ...
    // place order BRI Ceria
    // const [openBriCeria, setOpenBriCeria] = useState(false);
    // const [briCeriaData, setBriCeriaData] = useState(null);
    const handleBriCeria = (order_id) => {
        const state = { ...checkout };
        state.loading.order = true;
        setCheckout(state);
        getBriCeriaToken({
            ...
            if (res?.data?.getBriCeriaPaymentTokenByOrderId?.bri_ceria_token
                && res?.data?.getBriCeriaPaymentTokenByOrderId?.redirect_url) {
                // setBriCeriaData(res.data.getBriCeriaPaymentTokenByOrderId);
                // setOpenBriCeria(true);
                state.loading.order = false;
                setCheckout(state);
    ...
    const handlePlaceOrder = async () => {
        const { cart, customer: { firstname, lastname }, isGuest } = checkout.data;
        ...
        if (checkout.input_purchase_letter) {
            updateFormik({
                ...state.data.cart,
                ...state.input_purchase_letter,
            });
        }
        ...
        if (Object.keys(formValidation).length === 0 && formValidation.constructor === Object) {
            if (checkout.selected.delivery === 'pickup' && (checkout.error.pickupInformation || checkout.error.selectStore)) {
            ...
            } else if (formik.values.signatureName === '' && !formik.values.checkoutOtp) {
                state.loading.order = false;
                setCheckout(state);

                const msg = t('checkout:message:signatureNameValidation');
                handleOpenMessage({
                    variant: 'error',
                    text: msg,
                });
            } else if (!checkout?.input_purchase_letter) {
                state.loading.order = false;
                setCheckout(state);

                const msg = t('checkout:message:purchaseLetterValidation');
                handleOpenMessage({
                    variant: 'error',
                    text: msg,
                });
                ...
                let dataPostOrder = {
                    cartId: cart.id,
                    origin: originName,
                    signatureName: formik.values.signatureName,
                };

                if (!dataPostOrder.signatureName) {
                    dataPostOrder.signatureName = customerName.trim();
                }
                ...
                if (formik.values.orderComment !== '') {
                    ...
                    dataPostOrder = {
                        ...dataPostOrder,
                        spNumber: checkout.purchase_letter_value ? checkout.purchase_letter_value : '',
                    };
                }
                if (formik.values.orderComment !== '') {
                    result = await placeOrderWithOrderComment({
                        variables: dataPostOrder,
                    });
                } else {
                    await placeOrder({ variables: dataPostOrder })
                        .then((res) => {
                            result = res;
                        }).catch((err) => {
                            result = err;
                ...
                if (orderNumber && orderNumber !== '') {
                    ...
                    if (checkout.data.cart.selected_payment_method.code.match(/snap.*/)
                        && checkout.data.cart.selected_payment_method.code !== 'snapbilling') {
                        setOrderId(orderNumber);
                        await getSnapToken({ variables: { orderId: orderNumber } });
                    } else if (checkout.data.cart.selected_payment_method.code === 'briceriapayment') {
                        handleBriCeria(orderNumber);
                    } else if (
        ...
        return (
            <>
                ...
                {/* <ModalBriCeria
                    ...
                    setOpen={() => setOpenBriCeria(!openBriCeria)}
                /> */}
                <Traveloka3DSModal
                    open={openTraveloka}
